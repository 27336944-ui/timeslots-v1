...
Page<DetailPageData, DetailPageMethods>({
  ...
  async onRegenerateCode() {
    try {
      ...
      wx.showToast({ title: errorMsg(e) || '刷新失败', icon: 'none' });
      wx.hideLoading();
    }
  },
      ...
      success: async (res) => {
        if (res.confirm) {
          try {
            ...
            wx.showToast({ title: errorMsg(err) || '移除失败', icon: 'none' });
            wx.hideLoading();
          }
        }
      },
      ...
      success: async (res) => {
        if (res.confirm) {
          try {
            ...
            wx.showToast({ title: errorMsg(err) || '退出失败', icon: 'none' });
            wx.hideLoading();
          }
        }
      },
      ...
      success: async (res) => {
        if (res.confirm) {
          try {
            ...
            wx.showToast({ title: errorMsg(err) || '删除失败', icon: 'none' });
            wx.hideLoading();
          }
        }
      },
